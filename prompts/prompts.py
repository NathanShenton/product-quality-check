...
PROMPT_OPTIONS = {
    ...
    "COMPLETE: FNV Line-by-Line Estimator": {
        "prompt": (
            ...
            "        – Veg-based crisps/chips (e.g. plantain) with no % → **≥60 %** (use 60 % unless evidence suggests otherwise).\n"
            ...
            "4. For multi-flavour assortments or layered bars, score each sub-recipe separately then use the highest single-share FNV %, "
            "or a weighted average if pack proportions are given – still respecting the 100 % cap.\n\n"

            "–––––  INTERNAL SELF-CHECK  –––––\n"
            "Before you emit the JSON, run this checklist:\n"
            "• Re-scan **fnv_ingredients**. If it contains any excluded cereals, dairy, isolates, water or additives, REMOVE them and subtract their % from certain_fnv.\n"
            "• Re-verify (certain_fnv + presumptive_fnv) ≤ 100 and both values are whole integers.\n"
            "• Ensure all arithmetic shown in debug_notes matches the final numbers.\n"
            "If any correction was needed, overwrite the draft JSON with the fixed values **before responding**.\n\n"

            "–––––  OUTPUT GUIDELINES  –––––\n"
            "• **fnv_ingredients** – list every ingredient that contributed to either score, ordered by % impact.\n"
            "• **debug_notes** – show arithmetic or key assumptions, ≤20 words each, e.g. "
