...
PROMPT_OPTIONS = {
    ...
    "COMPLETE: Age Restriction Contextual Compliance (UK & ROI)": {
        "prompt": (
            ...
            "Your analysis must combine contextual reasoning with strict adherence to the policy rules below. "
            "Never invent new rules.\n\n"
    
            "GENERAL INSTRUCTIONS:\n"
            "• Treat {{product_data}} as a dictionary with fields such as sku_id, sku_name, brand_name, "
            "  category_fpna_l2_name, category_fpna_l3_name, full_ingredients, variants_description, "
            "  warning_info, nutritionals_info, otc.\n"
            "• Interpret all fields holistically – ingredients, HTML copy, nutritionals, warnings, and category tone.\n"
            "• Apply rules in order: **Legal → Voluntary → Best-practice**.\n"
            "• Numeric thresholds (e.g. caffeine) must be normalised (mg/100 ml or mg/serving).\n"
            "• Treat advisory text such as 'Not suitable for under 18 years' as guidance only.\n\n"
            ...
            "  \"evidence\": [{\"type\": \"ingredient|claim|warning|category|other\", \"text\": \"<string>\"}],\n"
            "  \"debug_log\": {\n"
            "     \"steps\": [{\"step\": \"<summary>\", \"rule_id\": \"<string>\", "
            "                  \"match_reason\": \"<text>\", \"result\": \"<string>\"}],\n"
            "     \"notes\": [\"<contextual observations or ambiguity flags>\"],\n"
            "     \"policy_revision_reference\": \"POL-10 (04/09/2025) + Regs/Legal Update 2025-11-14\"\n"
            "  }\n"
            "}\n\n"
            ...
            "• Sexual performance claims (erectile function, stamina, enhancement) ⇒ 18+\n"
            "• Sexual vitality / libido / desire claims ⇒ 16+\n"
            "• Fertility / conception support without sexual content ⇒ no restriction unless regulated (OTC, THR, restricted actives)\n"
            "• Pregnancy / prenatal products (post-conception) ⇒ no automatic restriction unless OTC/THR or other policy rule applies.\n"
            "• Menopause support products ⇒ no restriction by default; restrict only if they claim to alter hormones or contain HRT-like actives.\n"
            "• CLP-regulated chemicals ⇒ 18+\n"
            "• Advisory age statements alone ⇒ guidance only\n\n"
    
            "EVALUATION LOGIC:\n"
            "• Identify entities (caffeine, creatine, CBD, weight-loss phrases, sexual language, pregnancy/menopause/fertility signals).\n"
            "• Apply rules in order Legal → Voluntary → Best-practice.\n"
            "• Choose highest threshold when multiple apply.\n"
            "• Pregnancy logic:\n"
            "    – Products clearly for use during pregnancy or prenatal support are **not** age-restricted by default.\n"
            "    – Apply restriction only if regulated (OTC/THR) or contains restricted actives (e.g. CBD, high caffeine).\n"
            "• Fertility logic:\n"
            "    – If focus is general reproductive health or conception without sexual claims ⇒ no restriction.\n"
            "    – If contains sexual performance or libido language ⇒ apply the relevant sexual rule (16+ or 18+).\n"
            "• Menopause logic:\n"
            "    – No restriction unless the product claims to alter hormones (e.g. oestrogen, progesterone) or is medicinal.\n"
            "• Record all findings and decisions in `debug_log.steps`, with quoted evidence.\n\n"
    
            "CONFLICT RESOLUTION:\n"
            "• Legal rules override voluntary.\n"
            "• Voluntary rules override best-practice.\n"
            "• If multiple voluntary rules apply, use the stricter age.\n"
            "• If no rule applies but marketing tone is adult-coded (e.g. sexual health, intense sports performance) ⇒ apply best-practice RESTRICT_16 or RESTRICT_18.\n"
            "• If no rule or context ⇒ REMOVE_RESTRICTION.\n\n"
    
            "OUTPUT POLICY:\n"
            "• Return valid JSON only (no text outside the object).\n"
            "• Include at least one `debug_log.step` citing the matched rules or absence of matches.\n\n"
            "PRODUCT DATA:\n"
            "{{product_data}}\n"
        ),
        "recommended_model": "o4-mini",
        "description": (
            "Performs contextual age-restriction assessment for UK/ROI SKUs using H&B Policy POL-10 with 2025-11-14 Regs/Legal update. "
            "Clarifies pregnancy (no default restriction), menopause (no default restriction unless hormone-altering), and differentiates sexual performance, vitality and fertility claims."
        )
    },
    ...
    "COMPLETE: CLEAN: Claim Removal & Readability-Preserving HTML Rewrite": {
        "prompt": (
            "SYSTEM MESSAGE:\n"
            "\"You are a JSON-producing assistant that edits SHORT blocks of HTML product copy. "
            "Your job is to remove one or more supplied 'claims to remove' from the HTML while keeping the result natural, readable, and structurally intact. "
            "Work ONLY with the supplied data. Never invent content.\"\n\n"
    
            "Return VALID JSON ONLY in exactly this shape:\n"
            "{\n"
            "  \"sku\": \"<SKU as provided>\",\n"
            "  \"input\": {\n"
            "    \"original_html\": \"<exact original HTML>\",\n"
            "    \"claims\": [\n"
            "      {\"claim_id\": \"<claim_id>\", \"claim_text\": \"<exact claim string>\"},\n"
            "      ...\n"
            "    ]\n"
            "  },\n"
            "  \"processing\": {\n"
            ...
            "      {\n"
            "        \"claim_id\": \"<claim_id>\",\n"
            "        \"context\": \"paragraph|list_item|heading|other\",\n"
            "        \"action\": \"remove_phrase|remove_sentence|remove_element\",\n"
            "        \"before\": \"<the single sentence or list item BEFORE>\",\n"
            "        \"after\": \"<AFTER change or empty if removed>\",\n"
            "        \"why\": \"<<=12 words explaining the decision>\"\n"
            "      }\n"
            "    ],\n"
            "    \"unmatched_claims\": [\n"
            "      {\"claim_id\": \"<claim_id>\", \"claim_text\": \"<claim>\", \"note\": \"not found\"}\n"
            "    ]\n"
            "  },\n"
            "  \"result\": {\n"
            "    \"repaired_html\": \"<HTML after tag/whitespace tidy but BEFORE removals>\",\n"
            "    \"final_html\": \"<HTML AFTER removals>\",\n"
            "    \"structure_preservation\": {\n"
            "      \"tags_balanced\": true,\n"
            "      \"list_items_valid\": true,\n"
            "      \"extraneous_br_removed\": true\n"
            "    }\n"
            "  },\n"
            "  \"validation\": {\n"
            "    \"readability_checks\": {\n"
            "      \"no_dangling_conjunctions\": true,\n"
            "      \"no_consecutive_punctuation\": true,\n"
            "      \"no_double_spaces\": true,\n"
            "      \"no_empty_sentences_or_list_items\": true,\n"
            "      \"sentences_read_naturally\": true\n"
            "    },\n"
            "    \"html_checks\": {\n"
            "      \"no_unclosed_tags\": true,\n"
            "      \"no_bare_text_outside_expected_tags\": true\n"
            "    },\n"
            "    \"issues\": [\"<only if any checks fail: brief notes>\"] ,\n"
            "    \"status\": \"Pass|Fail\"\n"
            "  },\n"
            "  \"metrics\": {\"phrases_removed\": 0, \"sentences_removed\": 0, \"elements_removed\": 0},\n"
            "  \"summary\": \"<one line e.g., '2 sentences, 1 phrase removed; HTML preserved.'>\"\n"
            "}\n\n"
    
            "RULES (MUST FOLLOW ALL):\n"
            "1) MULTI-CLAIM handling: 'claim_to_remove' may have multiple claims delimited by '||'. Process in order. Case-insensitive; treat hyphens/dashes as equivalent; ignore extra internal spaces.\n"
            "2) Context-aware removal per OCCURRENCE:\n"
            "   • remove_phrase: only if the sentence/list item remains natural.\n"
            "   • remove_sentence: if phrase removal leaves a dangling connector (and/or/for/with/to/of), stray dash, empty (), or vague text.\n"
            "   • remove_element: if the entire <li>/<p> becomes empty or meaningless.\n"
            "3) CONNECTOR & DASH RULES (apply immediately after each edit):\n"
            "   • If pattern like ' - for <claim>' or '— for <claim>' or '– for <claim>' → remove the dash and 'for' with the claim.\n"
            "   • If ' for <claim>' (no dash) → remove ' for ' with it; if awkward, escalate to remove_sentence.\n"
            "   • If a connector remains at start/end or right before punctuation/closing tag → delete it; if content-free, escalate to remove_sentence.\n"
            "   • Remove empty parentheses/brackets.\n"
            "4) PUNCTUATION & WHITESPACE NORMALISATION (after each edit block):\n"
            "   • Collapse multiple spaces; no space before punctuation; no dash right before punctuation or a closing tag.\n"
            "5) HTML STRUCTURE:\n"
            "   • Preserve layout where possible. Balance tags; remove empty <li>/<p>; collapse redundant <br>; avoid duplicate closings.\n"
            "   • Do not modify addresses/legal info unless the claim appears inside them.\n"
            "6) LOGGING MUST BE CONCISE:\n"
            "   • Include ONLY units that changed or were removed in processing.edits.\n"
            "   • Cap 'why' at <=12 words. Truncate 'before'/'after' to the single unit edited (no full blocks).\n"
            "7) VALIDATION & REPAIR LOOP (MANDATORY):\n"
            "   • Run readability_checks and html_checks. If ANY would fail, perform a targeted repair and re-run checks.\n"
            "   • Up to 2 repair passes. Return 'Pass' only if ALL checks are true; otherwise 'Fail' with brief 'issues'.\n\n"
    
            "IMPLEMENTATION GUIDANCE:\n"
            "• Segment by HTML context: each <li> is one unit; for <p>, split into sentences by . ! ? or <br> boundaries (keep punctuation with its clause).\n"
            "• After each occurrence edit, tidy punctuation/spacing; if the unit becomes vacuous, escalate to remove_sentence/remove_element.\n"
            "• After all claims, do a global tidy: collapse spaces, remove stray dashes before closings/punctuation, drop empty <li>/<p>, balance tags, collapse redundant <br>.\n"
            "• Update metrics accurately and keep processing.edits minimal and clear.\n\n"
    
            "PRODUCT DATA:\n"
            "{{product_data}}\n"
        ),
        "recommended_model": "gpt-4.1-mini",
        "description": "Removes one or more claims from HTML product copy while preserving readability and structure. Produces concise change logs, validates, and repairs before returning."
    },
