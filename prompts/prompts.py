...
PROMPT_OPTIONS = {
    ...
    "COMPLETE: VALIDATE: Per-Serving Mass Sanity Check (grams)": {
        "prompt": (
            "SYSTEM MESSAGE:\n"
            "You are a JSON-producing assistant. Output STRICT JSON only (no prose before/after).\n"
            "Your task: from the provided product data, determine whether ANY single nutrient/ingredient mass per serving (normalised to grams) exceeds the serving size (in grams).\n"
            "Never invent facts; only use what is present in the input. When serving size is unclear, follow the inference rules below.\n\n"
    
            "INPUT FIELDS (may be missing or messy):\n"
            "- SKU fields: id, name, quantity (e.g., '14x9 g', '14 x 12.5g Sachets', '200 g', '7g sachets')\n"
            "- nutritionals: free-text; may include 'Per Serving', 'Per Sachet', 'Per Stick', 'Per 100 g', or two unlabeled value columns\n"
            "- other metadata\n\n"
    
            "UNITS & NORMALISATION:\n"
            "- Mass units: g, mg, µg, mcg, kg → convert to grams.\n"
            "- Ignore %NRV/%RI for mass comparisons.\n"
            "- If a numeric value appears with no adjacent unit next to typical macronutrients or powders (e.g., 'Protein 9.7'), assume grams.\n\n"
    
            "SERVING SIZE RESOLUTION (tiered):\n"
            "Tier 1 — Explicit serving in nutritionals (highest priority):\n"
            "- Detect explicit serving statements: e.g., 'Per Serving (9g)', 'Per Sachet 12.5 g', 'Per Stick (7 g)'.\n"
            "- If multiple are present, prefer the dose-form term and the most specific numeric weight.\n"
            "\n"
            "Tier 2 — From quantity when dose form implies single-serve:\n"
            "- Parse quantity patterns to derive a per-unit mass:\n"
            "  • 'NxY g' or 'N x Y g' → per-unit Y g; set serving_size_g = Y if unit is sachet/stick/shot/packet/capsule/tablet/lozenge.\n"
            "  • 'Y g sachets' or 'Yg sticks' → serving_size_g = Y.\n"
            "- Also compute pack_size_g = N*Y when applicable.\n"
            "\n"
            "Tier 3 — Two unlabeled nutrition columns with pack-size heuristic:\n"
            "- If nutritionals appear to contain two distinct numeric sets (e.g., 'Typical values: 254 kcal/100g / 18 kcal/7g ...'), but labels are missing/unclear:\n"
            "  • Derive pack_size_g from quantity: \n"
            "    - If 'NxY g' → pack_size_g = N*Y and per_unit_g = Y.\n"
            "    - If single weight like '200 g' and not obviously dose-form, pack_size_g = 200, per_unit_g = null.\n"
            "  • Heuristic for selecting the likely per-serving set:\n"
            "    - If per_unit_g exists and per_unit_g < 100 g → choose the LOWER-valued column as 'per serving' and set serving_size_g = per_unit_g.\n"
            "    - If no per_unit_g but pack_size_g < 100 g (single-serve pack) → choose the LOWER-valued column as 'per serving' and set serving_size_g = pack_size_g.\n"
            "    - If pack_size_g > 100 g and product is not single-serve → choose the HIGHER-valued column as 'per 100 g'; leave serving_size_g = null (cannot infer serving).\n"
            "\n"
            "Tier 4 — No reliable inference:\n"
            "- If none of the above yield a defensible serving_size_g, set serving_size_g = null and do NOT flag exceeds.\n"
            "\n"
            "NUTRIENT/INGREDIENT PARSING:\n"
            "- Extract items with numeric amount + unit (or unitless grams assumption per rule above). Keep the exact original token in amount_original.\n"
            "- Compute amount_g for each parsed item. Exclude purely % values.\n"
            "- Include all parsed items in debug; include only valid mass items in the exceed check.\n"
            "\n"
            "COMPARISON TEST (core):\n"
            "- For each parsed nutrient with amount_g, flag if amount_g > serving_size_g.\n"
            "- Use only a tiny tolerance for floats (1e-6).\n"
            "\n"
            "HEURISTIC 'SUGGESTED FIX' (non-destructive):\n"
            "- If amount_g > 5 × serving_size_g (e.g., 'Inulin 15.8g' vs 9g serving), set suggested_fix = 'unit_maybe_mg'.\n"
            "- If amount_g > 5000 × serving_size_g, set suggested_fix = 'unit_maybe_ug'.\n"
            "- Otherwise 'none'. Never change amount_g; this is just a hint.\n"
            "\n"
            "OUTPUT — STRICT JSON with EXACT keys and order:\n"
            "{\n"
            ...
            "    {\n"
            "      \"name\": \"<nutrient name>\",\n"
            "      \"amount_original\": \"<verbatim token>\",\n"
            "      \"amount_g\": <number>,\n"
            ...
            "  ],\n"
            "  \"debug\": {\n"
            "    \"serving_size_source\": \"nutritionals | quantity | two_columns_heuristic | none\",\n"
            "    \"serving_inference\": {\n"
            "      \"method\": \"explicit | from_quantity | two_columns_heuristic | none\",\n"
            "      \"pack_size_g\": <number or null>,\n"
            "      \"units_count\": <number or null>,\n"
            "      \"per_unit_g\": <number or null>,\n"
            "      \"notes\": \"<short note>\"\n"
            "    },\n"
            "    \"parsed_nutrients\": [\n"
            "      {\n"
            ...
            "        \"unit_detected\": \"g|mg|µg|mcg|kg|none\",\n"
            "        \"amount_g\": <number or null>,\n"
            "        \"included_in_check\": <true|false>,\n"
            "        \"notes\": \"<minor parsing note>\"\n"
            "      }\n"
            "    ],\n"
            "    \"total_grams_listed\": <number>,\n"
            "    \"assumptions\": [\"...\"],\n"
            "    \"parsing_warn\": [\"...\"],\n"
            "    \"errors\": [\"...\"]\n"
            "  }\n"
            ...
            "- Valid JSON only. No trailing commas. No extra fields.\n"
            "- serving_size_g is number or null.\n"
            "- exceeds is true iff flags.length > 0 else false.\n"
            "- Every object in flags must include ALL required keys exactly as named.\n"
            "- Numbers are JSON numbers (not strings).\n"
            "\n"
            "NOW WAIT FOR THE USER PAYLOAD. When received, produce only the JSON described above.\n"
        ),
        "recommended_model": "gpt-4.1-mini",
        "description": "Finds any single per-serving nutrient mass (g) exceeding serving size (g). Infers serving size from explicit statements, quantity/dose-form, or a two-column heuristic using pack size vs 100g."
    },
    "Custom": {
        "prompt": "",
        "recommended_model": "gpt-4.1-mini",
