...
PROMPT_OPTIONS = {
    ...
    "COMPLETE: VALIDATE: Per-Serving Mass Sanity Check (grams)": {
        "prompt": (
            ...
            "Your task: from the provided product data, determine whether ANY single nutrient/ingredient mass per serving (grams) exceeds the serving size (grams).\n"
            "Never invent facts; only use what is present in the input.\n\n"
    
            "INPUT DATA:\n"
            "- nutritionals: free-text (may include per serving, per sachet, per stick, per 100g/ml)\n"
            "- quantity: e.g., '14x9 g', '14x12.5g sachets', '200 g'\n"
            "- other metadata\n\n"
    
            "SERVING SIZE RESOLUTION:\n"
            "1) If nutritionals explicitly state serving (e.g., 'Per Serving (9g)', 'Per Sachet 12.5 g'), use that.\n"
            "2) Else if quantity is dose-form (sachet/stick/capsule/etc) like '14x9 g', infer serving_size_g = 9.\n"
            "3) Else if two columns appear (per 100g vs per Xg):\n"
            "   - If pack < 100 g or per-unit mass < 100 g, choose the LOWER column as per serving.\n"
            "   - If pack > 100 g, treat HIGHER column as per 100 g only (serving_size_g = null).\n"
            "4) Else if no reliable inference: serving_size_g = null, no exceeds.\n\n"
    
            "NUTRIENT PARSING RULES:\n"
            "- Extract only nutrients with a numeric amount + unit.\n"
            "- Mass units: g, mg, µg, mcg, kg → convert to grams.\n"
            "- If unit missing but looks like macronutrient/powder, assume grams.\n"
            "- Ignore kcal/kJ and %NRV.\n"
            "- If line contains 'yielding' or 'of which active', include in debug only (not checked).\n"
            "- If explicitly per 100 g/ml, exclude from checks.\n\n"
    
            "COMPARISON:\n"
            "- Only compare per-serving items.\n"
            "- Flag if amount_g > serving_size_g + 1e-6.\n"
            "- suggested_fix:\n"
            "   - if >5× serving → 'unit_maybe_mg'\n"
            "   - if >5000× serving → 'unit_maybe_ug'\n"
            "   - else 'none'.\n"
            "- confidence:\n"
            "   - 0.9–1.0 if explicit serving + unit clear\n"
            "   - 0.5–0.7 if inferred (quantity/two-column)\n"
            "   - ≤0.5 if ambiguous.\n\n"
    
            "OUTPUT (minimal by default):\n"
            "{\n"
            ...
            "OPTIONAL DEBUG MODE (only if user passes 'debug=true' in payload):\n"
            "{\n"
            "  \"serving_size_source\": \"nutritionals | quantity | heuristic | none\",\n"
            "  \"assumptions\": [\"...\"],\n"
            "  \"parsing_warn\": [\"...\"],\n"
            "  \"errors\": [\"...\"]\n"
            "}\n\n"
            ...
            "VALIDATION:\n"
            "- Always valid JSON. No trailing commas. No extra fields.\n"
            "- exceeds = true iff flags.length > 0 else false.\n"
            "- serving_size_g is number or null.\n"
            "- Numbers are JSON numbers, not strings.\n"
        ),
        "recommended_model": "gpt-4.1-mini",
        "description": "Efficiently checks if any per-serving nutrient mass exceeds the serving size. Returns only flags/errors unless debug is requested."
    },
    "Custom": {
        "prompt": "",
        "recommended_model": "gpt-4.1-mini",
