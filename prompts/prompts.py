...
PROMPT_OPTIONS = {
    ...
    "AUDIT: Free-From Claim Check": {
        "prompt": (
            ...
            "   - Keep only entries that exactly match (case-insensitive) one of supported_claims.\n\n"
            "2) Preprocess text fields:\n"
            "   - Remove all HTML tags from full_ingredients and warning_info.\n"
            "   - Convert both cleaned fields to lowercase.\n\n"
            "3) Conflict detection (strict root-scoped matching):\n"
            "   - For each claim, derive its root key by stripping the trailing ' Free' (e.g. 'Gluten Free' → 'Gluten').\n"
            "   - Load synonyms[root].\n"
            "   - For each phrase in that array, perform an exact whole-word match in the cleaned text, allowing only:\n"
            "       • an optional trailing plural 's'\n"
            "       • optional hyphens or spaces within the phrase\n"
            "   - Do NOT match any term not explicitly in synonyms[root], and do NOT consider other synonyms arrays.\n"
            "   - If a match is found, record that claim as violated.\n\n"
            "4) Construct outputs:\n"
            "   - violated_claims: comma-separated list of each violated claim, in original diet_info order.\n"
            "   - debug_matches: array of strings:\n"
            ...
            "   - Each claim in violated_claims must have exactly one corresponding entry in debug_matches, and vice-versa.\n"
            "   - Do NOT include any claim without a genuine exact-match synonym.\n\n"
            "7) Final step:\n"
            "Send only the JSON object exactly as specified—no prose, no extra fields."
        ),
        "recommended_model": "gpt-4.1-mini",
        "description": "Checks product free-from claims against ingredient and warning statements using strict root-scoped synonym matching, HTML stripping, and no scratchpad."
    },
