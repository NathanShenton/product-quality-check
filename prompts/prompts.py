...
PROMPT_OPTIONS = {
    ...
    "COMPLETE: FNV Line-by-Line Estimator": {
        "prompt": (
            ...
            "4. For multi-flavour assortments … still respecting the 100 % cap.\n\n"

            "5. 🔄 **Self-check (cereal-sanity pass)** – immediately after you build the JSON.\n"
            "    • Compare every entry in **fnv_ingredients** against the EXCLUSION list "
            "{wheat, barley, rye, oats, corn, maize, rice, quinoa, spelt, sorghum, millet, teff}.\n"
            "    • If any match →\n"
            "        – Remove it from **fnv_ingredients**.\n"
            "        – Subtract its % from **certain_fnv** if you had (wrongly) added it.\n"
            "        – If it drove a presumptive guess, reduce **presumptive_fnv** by the amount you assumed "
            "(never below 0).\n"
            "        – Append a `debug_notes` item: \"❗ removed cereal/pseudo: <name> — scores corrected\".\n"
            "    • Re-run rule 3 to confirm (certain + presumptive ≤ 100; round up).\n\n"
