# artwork_processing_warnings_advisory.py
...
# Strict classifier for safety text
...
# ─────────────────────────────────────────────────────────────────────────────
# Heuristic region finder (OCR-only; for locating, not classifying)
# ─────────────────────────────────────────────────────────────────────────────
...
def _gpt_bbox_locator_warnadv_multi(client, img: Image.Image, model: str) -> List[Tuple[int,int,int,int]]:
...
def process_artwork_warnings_advisory(
    ...
    *,
    ...
    if is_pdf:
        ...
        # Normalize flat artwork to main consumer panel before locating safety text
    ...
    # top-up from staple phrases on full page (prefers full_raw if available)
    ...
    if boxes:
        ...
        consistency_ok = (again_txt == target) or (consist_ratio >= 98.0)
